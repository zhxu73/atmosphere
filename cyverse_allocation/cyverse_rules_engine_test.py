...
class CyverseRulesEngineTest(TestCase):
    ...
    def test_total_usage(self):
        ...
        allocation_source = create_allocation_source(
            name='TestSource', compute_allowed=1000, timestamp=ts
        )
        ...
        workflow1.assign_allocation_source_to_user(
            allocation_source, timestamp=ts + timedelta(minutes=10)
        )
        instance = workflow1.create_instance(
            start_date=ts + timedelta(minutes=30)
        )
        workflow1.create_instance_status_history(
            instance, start_date=ts + timedelta(days=30), status='suspend'
        )
        workflow1.assign_allocation_source_to_instance(
            allocation_source, instance, timestamp=ts + timedelta(minutes=40)
        )
        ...
        workflow2.assign_allocation_source_to_user(
            allocation_source, timestamp=ts + timedelta(minutes=10)
        )
        instance = workflow2.create_instance(
            start_date=ts + timedelta(minutes=30)
        )
        workflow2.assign_allocation_source_to_instance(
            allocation_source, instance, timestamp=ts + timedelta(minutes=40)
        )
        ish_start_date = ts + timedelta(minutes=120)
        workflow2.create_instance_status_history(
            instance, start_date=ish_start_date, status='suspended'
        )
