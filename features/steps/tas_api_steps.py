...
def we_clear_the_local_cache(context):
    ...
    context.test.assertDictEqual(
        jetstream_allocation.TASAPIDriver.username_map, {}
    )
    context.test.assertListEqual(
        jetstream_allocation.TASAPIDriver.user_project_list, []
    )
    context.test.assertListEqual(
        jetstream_allocation.TASAPIDriver.project_list, []
...
def xsede_to_tacc_username_mappings(context):
    context.xsede_to_tacc_username_mapping = dict(
        row.cells for row in context.table
    )
...
def these_tacc_usernames_for_tas_projects(context):
    ...
    for tas_project, tacc_usernames in context.tas_project_to_tacc_username_mapping.iteritems(
        ...
        for tacc_username in tacc_usernames:
            user_tas_projects = context.tacc_username_to_tas_project_mapping.get(
                tacc_username, set()
            )
            user_tas_projects.add(tas_project)
            context.tacc_username_to_tas_project_mapping[tacc_username
                                                        ] = user_tas_projects
...
def we_get_all_projects(context):
    with mock.patch.multiple(
        ...
        mock_methods['tacc_api_post'].side_effect = _make_mock_tacc_api_post(
            context
        )
        mock_methods['tacc_api_get'].side_effect = _make_mock_tacc_api_get(
            context
        )
        all_projects = context.driver.get_all_projects()
    context.test.assertListEqual(all_projects, context.tas_projects)
    context.test.assertListEqual(
        context.driver.project_list, context.tas_projects
    )
...
def should_have_allocation_sources(context):
    expected_allocation_sources = dict(
        (
            row.cells[0],
            Decimal(row.cells[1]),
        ) for row in context.table
    ...
    allocation_sources = {
        item.name: item.compute_allowed
        for item in AllocationSource.objects.all().order_by('name')
    }
    context.test.assertDictEqual(
        expected_allocation_sources, allocation_sources
    )
...
def should_have_allocation_source_snapshots(context):
    ...
    allocation_source_snapshots = {
        item.allocation_source.name: [item.compute_used, item.compute_allowed]
        for item in AllocationSourceSnapshot.objects.
        select_related('allocation_source').all()
    }
    context.test.assertDictEqual(
        expected_allocation_source_snapshots, allocation_source_snapshots
    )


@step(u'we should have the following user allocation sources')
def should_have_user_allocation_sources(context):
    expected_user_allocation_sources = [
        (
            row.cells[0],
            row.cells[1] or None,
        ) for row in context.table
    ...
    user_allocation_sources = [
        (
            item['user__username'],
            item['allocation_source__name'],
        ) for item in UserAllocationSource.objects.all().order_by(
            'user__username', 'allocation_source__name'
        ).values('user__username', 'allocation_source__name')
    ]
    context.test.assertItemsEqual(
        expected_user_allocation_sources, user_allocation_sources
    )
...
def should_have_user_allocation_source_snapshots(context):
    raw_expected_allocation_source_snapshots = [
        dict(zip(row.headings, row.cells)) for row in context.table
    ]
    ...
    import core.models
    raw_user_allocation_source_snapshots = core.models.UserAllocationSnapshot.objects.all(
    ).values(
        'user__username', 'allocation_source__name', 'compute_used', 'burn_rate'
    ...
    context.test.maxDiff = None
    context.test.assertItemsEqual(
        expected_allocation_source_snapshots, user_allocation_source_snapshots
    )
...
def should_have_following_events(context, event_name=None):
    ...
    for expected_event in expected_events:
        ...
        if hasattr(context, 'persona'
                  ) and isinstance(context.persona, collections.Mapping):
    ...
    for event in events:
        ...
        event['timestamp'] = datetime.datetime.strftime(
            event['timestamp'], '%Y-%m-%d %H:%M:%S%z'
        )
        actual_event_content = [
            dump_field(event[field]) for field in fields_we_care_about
        ]
        actual_event_row = '| {} |'.format(' | '.join(actual_event_content))
        actual_events_rows.append(actual_event_row)
    print('actual_events_rows:')
    print(
        '(In case of unexpected events this will provide an easy way to copy valid table values from the console)'
    )
...
def the_user_should_be_valid(context, user_is_valid):
    ...
    with mock.patch.multiple(
        ...
        with django.test.override_settings(
            ...
            import core.plugins
            core.plugins.ValidationPluginManager.list_of_classes = getattr(
                django.conf.settings, 'VALIDATION_PLUGINS', []
            )
            context.test.assertListEqual(
                core.plugins.ValidationPluginManager.list_of_classes,
                overridden_validation_plugins
            )
            try:
...
def we_ensure_local_allocation_is_created_or_deleted(context):
    ...
    import core.models
    context.test.assertIsInstance(user, core.models.AtmosphereUser)
    active_allocation_count = core.models.UserAllocationSource.objects.filter(
        user=user
    ).count()
    example = dict(
        zip(context.scenario._row.headings, context.scenario._row.cells)
    ...
    allocation_source = api.tests.factories.AllocationSourceFactory.create(
        name='TG-ALT_{}'.format(user.username), compute_allowed=3000
    )
    context.test.assertIsInstance(
        allocation_source, core.models.AllocationSource
    )

    user_allocation_source = api.tests.factories.UserAllocationSourceFactory.create(
        user=user, allocation_source=allocation_source
    )
    context.test.assertIsInstance(
        user_allocation_source, core.models.UserAllocationSource
    )
