...
def main():
    ...
    parser.add_argument(
        "--host", help="The OpenStack compute node to launch"
        " instances on."
    )
    parser.add_argument(
        "--name", help="The OpenStack compute node to launch"
        " instances on."
    )
    parser.add_argument(
        "--provider-list",
    ...
    parser.add_argument(
        "--size", help="Atmosphere size to use when"
        " launching instances."
    )
    parser.add_argument(
        "--size-list", action="store_true", help="List of size names and IDs"
    )
    parser.add_argument(
        "--skip-deploy",
        action="store_true",
    ...
    parser.add_argument(
        "--count", default=1, type=int, help="Number of instances to launch."
    )
    parser.add_argument(
        "--users",
        help="Atmosphere usernames to launch with, in a comma-separated-list."
    )
...
def sort_most_used_machines(provider, limit=0, offset=0):
    ...
    for source in query:
        ...
        ).annotate(instance_count=Count('instance_source__instances'))
    return results
...
def launch(
    user, name_prefix, provider, machines, size, host, skip_deploy, count
):
    ...
    for c in range(0, count):
        for machine in machines:
            machine_count += 1
            gen_name = "%s v.%s" % (
                machine.application.name, machine.application_version.name
            )
            if name_prefix:
