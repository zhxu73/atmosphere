...
def _parse_args():
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Don't make changes, only print what would be synced"
    )
    parser.add_argument(
        "--limit-app-ids",
        type=int,
        ...
        metavar="APP_ID",
        help=
        "Limit synchronization to this space-separated list of application IDs "
        "(used for testing purposes)"
    )
    parser.add_argument(
        "--irods-conn",
        type=str,
        metavar="irods://user:password@host:port/zone",
        help=
        "iRODS connection string in the form of irods://user:password@host:port/zone"
    )
    parser.add_argument(
        "--irods-collections",
        type=json.loads,
        ...
        help=
        "JSON mapping each provider ID to an iRODS collection containing images"
    )
...
def main(
    ...
    glance_client_versions=None,
    dry_run=False,
    limit_app_ids=False,
    irods_conn=None,
    irods_collections=None
):
    ...
    if any([irods_conn, irods_collections]):
        if all([irods_conn, irods_collections]):
            for key in irods_collections.keys():
                ...
                assert (
                    type(irods_path) == unicode and len(irods_path) > 1
                    and irods_path[0] == "/"
                )
            assert (master_provider_id in irods_collections.keys())
        else:
            raise Exception(
                "If using iRODS transfer then irods_conn and irods_collections must be defined"
            )
    ...
    replica_provs = [
        core.models.Provider.objects.get(id=replica_id)
        for replica_id in replica_provider_ids
    ]

    if limit_app_ids:
        apps = core.models.Application.objects.filter(
            end_date__isnull=True
        ).filter(pk__in=limit_app_ids)
    else:
        ...
        if replicate_app:
            ...
            for replica_prov in replica_provs:
                if not _app_complete_on_provider(app, replica_prov):
                    logging.info(
                        "Migrating application {0} to provider {1}".format(
                            app, replica_prov
                        )
                    )
                    if not dry_run:
                        ...
                        try:
                            application_to_provider.main(
                                ...
                                src_glance_client_version=glance_client_versions
                                .get(str(master_prov.id))
                                if glance_client_versions else None,
                                dst_glance_client_version=glance_client_versions
                                .get(str(replica_prov.id))
                                if glance_client_versions else None
                            )
                            logging.info(
                                "Migrated application {0} to provider {1}".
                                format(app, replica_prov)
                            )
                        except Exception as e:
                            logging.error(
                                "Encountered exception while running application_to_provider:"
                            )
                            logging.exception(e)
                    else:
                        # Dry run
                        if irods_collections and (
                            replica_prov.id in irods_collections.keys()
                        ):
                            print(
                                "Sync application ID {0} to replica provider {1} using iRODS transfer -- "
                                "source collection {2} -- destination collection {3}"
                                .format(
                                    app.id, replica_prov.id,
                            ...
                            print(
                                "Sync application ID {0} to replica provider {1}"
                                .format(app.id, replica_prov.id)
                            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.info(
            "Running application_sync_providers with the following arguments:\n{0}"
            .format(str(args_for_log_output))
        )
        main(
            args.master_provider_id,
