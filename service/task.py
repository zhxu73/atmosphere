...
def deploy_init_task(
    ...
    **kwargs
    ...
    return deploy_init_to.apply_async(
        (
            driver.__class__, driver.provider, driver.identity, instance.alias,
            identity, username, password, redeploy, deploy
        ),
        immutable=True
    )
...
def mount_volume(
    ...
    **kwargs
    ...
    logger.info(
        "device_location:%s --> mount_location: %s" % (device, mount_location)
    )
    try:
        vol = driver.get_volume(volume_id)
        ...
        mount_chain = _get_mount_chain(
            core_identity, driver, instance_id, volume_id, device,
            mount_location
        )
...
def _get_umount_chain(driver, instance_id, volume_id, detach_task=None):
    ...
    pre_umount_status = update_volume_metadata.si(
        driverCls, provider, identity, volume_id, {'tmp_status': 'unmounting'}
    )
    ...
    pre_umount_status.link_error(
        mount_failed.s(driverCls, provider, identity, volume_id, True)
    )
    umount.link_error(
        mount_failed.s(driverCls, provider, identity, volume_id, True)
    )
