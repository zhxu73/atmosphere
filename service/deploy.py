...
def ansible_deployment(
    ...
    logger = create_instance_logger(
        deploy_logger, instance_ip, username, instance_id
    )
    ...
    shared_users = list(
        AtmosphereUser.users_for_instance(instance_id).values_list(
            'username', flat=True
        )
    )
    if not shared_users:
    ...
    if raise_exception:
        raise_playbook_errors(
            playbook_results, instance_id, instance_ip, hostname
        )
...
def deploy_unmount_volume(instance_ip, username, instance_id, device):
    """
    Use service.ansible to mount volume to an instance.
    """
    extra_vars = {"src": device}
    playbooks_dir = settings.ANSIBLE_PLAYBOOKS_DIR
    playbooks_dir = os.path.join(playbooks_dir, 'instance_actions')
...
def deploy_check_volume(
    instance_ip, username, instance_id, device, device_type='ext4'
):
    """
...
def instance_deploy(instance_ip, username, instance_id, limit_playbooks=[]):
    """
    Use service.ansible to deploy to an instance.
    """
...
def user_deploy(instance_ip, username, instance_id, first_deploy=True):
    ...
    if not instance.project:
        raise Exception(
            "Expected this instance to have a project, found None: %s" %
            instance
        )
    ...
    return raise_playbook_errors(
        playbook_results, instance_id, instance_ip, hostname
    )


def run_utility_playbooks(
    instance_ip,
    username,
    instance_id,
    limit_playbooks=[],
...
def user_deploy_install(
    instance_ip, username, instance_id, install_action, install_args
):
    """
...
def check_ansible():
    ...
    if not exists:
        logger.warn(
            "Ansible is not configured. Verify your "
...
def configure_ansible(debug=False):
    ...
    if settings.ANSIBLE_CONFIG_FILE:
        ...
        os.environ[
            "PYTHONOPTIMIZE"
        ] = "1"    #NOTE: Required to run ansible2 + celery + prefork concurrency
    ...
    * INSTANCE_HOSTNAMING_DOMAIN (Required if you use `%(domain)s`)
    ...
    try:
        provider = Provider.objects.get(
            instancesource__instances__provider_alias=instance_id
        )
        hostname_format_str = provider.cloud_config['deploy']['hostname_format']
    except Provider.DoesNotExist:
        logger.warn(
            "Using an instance %s that is *NOT* in your database. Cannot determine hostnaming format. Using IP address as hostname."
        )
        return raw_hostname(ip)
    except (KeyError, TypeError):
        logger.warn(
            "Cloud config ['deploy']['hostname_format'] is missing -- using IP address as a hostname."
        )
        return raw_hostname(ip)
    ...
    try:
        if all(
            (str_val not in hostname_format_str)
            for str_val in ['one', 'two', 'three', 'four']
        ):
            raise ValueError(
        ...
        logger.exception(
            "Invalid instance_hostname_format: %s" % hostname_format_str
        )
        return raw_hostname(ip)
...
def raise_playbook_errors(
    playbook_results, instance_id, instance_ip, hostname, allow_failures=False
):
    """
    ...
    if error_message:
        msg = "Instance: %s IP:%s %s - %s" % (
            ...
            'Hostname: ' + hostname if hostname else "", error_message
        )
