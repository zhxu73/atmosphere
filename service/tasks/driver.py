...
def deploy_failed(
    ...
    try:
        celery_logger.debug(
            "deploy_failed task started at %s." % datetime.now()
        )
        celery_logger.info("failed task context=%s" % (context, ))
        celery_logger.info("exception_msg=%s" % (exception_msg, ))
        ...
        # Send deploy email
        celery_logger.debug(
            "deploy_failed task finished at %s." % datetime.now()
        )
    except Exception as exc:
        celery_logger.warn(exc)
...
def deploy_init_to(
    ...
    **kwargs
    ...
    try:
        celery_logger.debug(
            "deploy_init_to task started at %s." % datetime.now()
        )
        ...
        celery_logger.debug(
            "deploy_init_to task finished at %s." % datetime.now()
        )
    except SystemExit:
        celery_logger.exception(
            "System Exits are BAD! Find this and get rid of it!"
        )
...
def get_idempotent_deploy_chain(
    driverCls, provider, identity, instance, core_identity, username
):
    """
    ...
    if instance_status in [
        'suspended', 'stopped', 'paused', 'shutoff', 'shelved',
        ...
        '', 'redeploying', 'deploying', 'deploy_error', 'user_deploy_error'
    ]:
        celery_logger.info(
...
def get_remove_status_chain(driverCls, provider, identity, instance):
    ...
    remove_status_task = update_metadata.si(
        driverCls, provider, identity, instance.id, final_update, replace
    )
...
def print_chain(start_task, idx=0):
    ...
    count = idx + 1
    signature = "\n%s Task %s: %s(args=%s) " % (
        "  " * (idx), count, start_task.task, start_task.args
    )
...
def get_chain_from_active_with_ip(
    ...
    if redeploy:
        celery_logger.warn(
            "WARNING: 'Redeploy' as an individual option is DEPRECATED, as 'ansible' is idempotent"
        )
    start_chain = None
...
def get_deploy_chain_second_half(
    ...
    check_vnc_task.link(
        deploy_user_task
    )    # this line and below, user can create a failure.
...
def destroy_instance(instance_alias, user, core_identity_uuid):
    ...
    try:
        celery_logger.debug(
            "destroy_instance task started at %s." % datetime.now()
        )
        core_instance = instance_service.destroy_instance(
            user, core_identity_uuid, instance_alias
        )
        celery_logger.debug(
            "destroy_instance task finished at %s." % datetime.now()
        )
...
def _deploy_ready_failed_email_test(
    driver, instance_id, exc_message, current_request, task_class
):
    """
    ...
    **celery_task_args
    ...
    try:
        ...
        if len(instance._node.private_ips) >= 2:
            _update_floating_ip_to_active_fixed_ip(
                driver, instance, core_identity_uuid
            )
    except (BaseException, Exception) as exc:
        celery_logger.exception(exc)
    ...
    try:
        ...
        _update_status_log(
            instance, "Ansible Finished (ready test) for %s." % instance.ip
        )
        celery_logger.debug(
            "deploy_ready_test task finished at %s." % datetime.now()
        )
...
def _deploy_instance_for_user(
    ...
    **celery_task_args
    ...
    try:
        celery_logger.debug(
            "_deploy_instance_for_user task started at %s." % datetime.now()
        )
    ...
    try:
        ...
        user_deploy(
            instance.ip, username, instance_id, first_deploy=first_deploy
        )
        _update_status_log(instance, "Ansible Finished for %s." % instance.ip)
        celery_logger.debug(
            "_deploy_instance_for_user task finished at %s." % datetime.now()
        )
    except AnsibleDeployException as exc:
        celery_logger.exception(exc)
...
def check_web_desktop_task(
    driverCls, provider, identity, instance_alias, *args, **kwargs
):
    """
    """
    try:
        celery_logger.debug(
            "check_web_desktop_task started at %s." % datetime.now()
        )
        driver = get_driver(driverCls, provider, identity)
        instance = driver.get_instance(instance_alias)
        if not instance:
        ...
        if retry_count > 2:
            should_raise = False
        playbook_results = run_utility_playbooks(
            instance.ip,
            username,
        ...
        core_instance.save()
        celery_logger.debug(
            "check_web_desktop_task finished at %s." % datetime.now()
        )
        ...
        celery_logger.warn(
            "check_web_desktop_task failed: Instance %s no longer exists" %
            instance_alias
        )
    except (BaseException, Exception) as exc:
        celery_logger.exception(exc)
...
@task(name="add_security_group_task", max_retries=5, default_retry_delay=10)
def add_security_group_task(
    driverCls, provider, core_identity, instance_alias, *args, **kwargs
):
    """
    ...
    try:
        ...
        raise Exception(
            "Cannot add the security group to the instance usng nova"
        )
    return True


@task(name="check_process_task", max_retries=2, default_retry_delay=15)
def check_process_task(
    driverCls, provider, identity, instance_alias, *args, **kwargs
):
    """
    ...
    try:
        celery_logger.debug(
            "check_process_task started at %s." % datetime.now()
        )
        driver = get_driver(driverCls, provider, identity)
        instance = driver.get_instance(instance_alias)
        if not instance:
        ...
        playbook_results = run_utility_playbooks(
            instance.ip,
            username,
            instance_alias, ["atmo_check_vnc.yml"],
        ...
        core_instance.save()
        celery_logger.debug(
            "check_process_task finished at %s." % datetime.now()
        )
        return vnc_enabled
    except AnsibleDeployException as exc:
        ...
        celery_logger.warn(
            "check_process_task failed: Instance %s no longer exists" %
            instance_alias
        )
    except (BaseException, Exception) as exc:
        celery_logger.exception(exc)
...
def update_metadata(
    ...
    try:
        celery_logger.debug(
            "update_metadata task started at %s." % datetime.now()
        )
        driver = get_driver(driverCls, provider, identity)
        instance = driver.get_instance(instance_alias)
        if not instance:
...
def add_floating_ip(
    ...
    **kwargs
    ...
    try:
        celery_logger.debug(
            "add_floating_ip task started at %s." % datetime.now()
        )
        core_identity = Identity.objects.get(uuid=core_identity_uuid)
        ...
        if floating_ips:
            ...
            selected_floating_ip = instance_floating_ips[
                0] if instance_floating_ips else None

        if selected_floating_ip:
            floating_ip_addr = selected_floating_ip["floating_ip_address"]
            celery_logger.debug(
                "Skip floating IP add:"
                " address %s already exists for Instance %s", floating_ip_addr,
                instance_alias
            )
            ...
            if core_identity.provider.cloud_config['network']['topology'] \
                ...
                admin_neutron = instance_service._to_network_driver(
                    admin_identity
                ).neutron
                routers = admin_neutron.list_routers(retrieve_all=True
                                                    )['routers']
                public_router = None
                ...
                if not public_router:
                    raise Exception(
                        "Could not find a router matching"
                        " public_router name {} for user {}".format(
                            public_router_name,
            ...
            celery_logger.debug(
                "Created new floating_ip_address - %s" % floating_ip_addr
            )
        ...
        # to vxlan
        instance_ports = network_driver.list_ports(device_id=instance.id)
        ...
        celery_logger.info(
            "Assigned IP:%s - Hostname:%s" % (floating_ip, hostname)
        )
        # End
        celery_logger.debug(
            "add_floating_ip task finished at %s." % datetime.now()
        )
        ...
        if 'no more ip addresses available' in floating_ip_err.message.lower():
            celery_logger.exception(
                "Error occurred while assigning a floating IP"
            )
        countdown = min(2**current.request.retries, 128)
        add_floating_ip.retry(exc=floating_ip_err, countdown=countdown)
...
def clean_empty_ips(driverCls, provider, identity, *args, **kwargs):
    try:
        celery_logger.debug(
            "remove_floating_ip task started at %s." % datetime.now()
        )
        driver = get_driver(driverCls, provider, identity)
        ips_cleaned = driver._clean_floating_ip()
        celery_logger.debug(
            "remove_floating_ip task finished at %s." % datetime.now()
        )
        return ips_cleaned
    except Exception as exc:
        celery_logger.warn(exc)
...
def add_os_project_network(core_identity, *args, **kwargs):
    try:
        celery_logger.debug(
            "add_os_project_network task started at %s." % datetime.now()
        )
        account_driver = get_account_driver(core_identity.provider)
        account_driver.create_network(core_identity)
        celery_logger.debug(
            "add_os_project_network task finished at %s." % datetime.now()
        )
    except Exception as exc:
...
def check_image_membership():
    try:
        celery_logger.debug(
            "check_image_membership task started at %s." % datetime.now()
        )
        update_membership()
        celery_logger.debug(
            "check_image_membership task finished at %s." % datetime.now()
        )
    except Exception as exc:
...
def update_membership_for(provider_uuid):
    ...
    if provider.type.name.lower() == 'openstack':
        ...
        celery_logger.warn(
            "Encountered unknown ProviderType:%s, expected"
            " [Openstack] " % (provider.type.name, )
        )
        return
    if not acct_driver:
        raise Exception(
            "Encountered error creating driver -- check 'get_account_driver'"
        )
    images = acct_driver.list_all_images()
    changes = 0
    for img in images:
        pm = ProviderMachine.objects.filter(
            instance_source__identifier=img.id,
        ...
        if img.get('visibility', '') is not 'public':
            ...
            if members:
                celery_logger.info(
                    "Application for PM:%s used to be private."
    ...
    for (instance_id, link_results) in linktest_results.items():
        try:
            ...
            if link_results['vnc'] != instance.vnc:
                celery_logger.debug(
                    'Change Instance %s VNC %s-->%s' %
                    (instance, instance.vnc, link_results['vnc'])
                )
...
def _update_floating_ip_to_active_fixed_ip(
    driver, instance, core_identity_uuid
):
    """
    ...
    floating_ips = network_driver.list_floating_ips()
    instance_floating_ips = [
        fip for fip in floating_ips if fip.get("instance_id", '') == instance_id
    ]
    selected_floating_ip = instance_floating_ips[
        0] if instance_floating_ips else None
    if selected_floating_ip and port_id:
        ...
        if previous_port_id != port_id:
            ...
            selected_floating_ip = network_driver.neutron.update_floatingip(
                selected_floating_ip['id'],
                {'floatingip': {
                    'port_id': port_id
                }}
            )
...
def _select_port_id(network_driver, driver, instance):
    ...
    fixed_ip_ports = [
        p
        for p in network_driver.list_ports() if p['device_id'] == instance_alias
    ]
    active_fixed_ip_ports = [
        p for p in fixed_ip_ports if 'ACTIVE' in p['status']
    ]
